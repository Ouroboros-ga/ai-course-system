...
`https://zsitai.xyz/`(公网入口;SSH 管理地址 `103.36.223.177`)。旧地址
`http://47.99.97.154/` 已随原服务器退租失效,文档中残留引用以本段为准。
Agent 在不确定服务真实行为时,允许对其做**只读**
访问(HTTP GET、健康检查、页面查看),不做写入或压测。开发基线分支为
`dev-liu`(`feature/xh202620` 已于 2026-09-03 合并进来,merge `67f94026`)。

服务器只读诊断入口:`ssh root@103.36.223.177`(密钥已配置)。部署根目录
`/opt/smartcarb/`;常驻服务 `smartcarb-backend`(:8000)、`nexus-runtime`(:8300)、
`repro-runtime`(:8401);演示账号见 `/opt/smartcarb/demo-student-accounts.csv`(合成数据)。
...
已部署服务地址为 `https://zsitai.xyz/`(SSH 管理地址 `103.36.223.177`)。编码智能体在不确定服务真实行为
(路由是否注册、页面是否可用、契约是否匹配)时,可直接对该地址做**只读**
访问(GET 请求、健康检查、查看页面);这属于只读诊断,不构成部署或变更授权,
不得对该地址做写入、压测或批量抓取。

读取服务器配置时只取**键名**用于判断"是否已配置",**不得输出** `.env` 的
值、Token、API Key 或任何密钥内容;进程环境变量(如 `/proc/<pid>/environ`)
一律不读取,避免密钥出域。
...
10. Quick Reproduction 中未知 GitHub Repo 视为不可信代码:不在 Backend 或
    ...
    迁移后为新机 103.36.223.177 同宿主机独立容器/独立网络)
...
6. 单文件超 50MB 不入库（GitHub 100MB 硬上限,双远端同步会断）;大权重走
   下载链接＋SHA256 记录,不用 LFS 不改历史（2026-09-11 星火 LoRA 教训）。
