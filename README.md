# SmartCarb —— 让课程回应学习：知识图谱驱动的可追溯智能教学系统
...
> **能力状态口径**：全文对外仅使用"已部署"与"规划能力"两种能力状态——"已部署"表示能力已进入远端运行环境并形成业务入口或服务调用链；"规划能力"表示尚无完整闭环，只能作为后续方向。量化效果与性能以专项实验结果为准。真实实现以代码、注册路由、数据库迁移、契约测试和浏览器手工行为为准；规划文档不能替代可运行证据。详见 [docs/DOCUMENTATION_INDEX.md](docs/DOCUMENTATION_INDEX.md)。
...
SmartCarb 是一套面向泛雅平台的智能体协作型全场景智慧教学系统，核心思想是**让课程回应学习**。系统以课程知识图谱为技术基座，把各科课件与历史文档带来源地解析，形成可信、可用、可追溯的知识结构；教师导入的课程知识库与图谱检索结合，为智能体提供精确的专业课件检索能力；教育智能体依据六维认知数据理解学生怎么学、学到什么程度、学习意愿如何，据此开展促学、导学、督学与办学，让智能体跟着学生一起进步。

系统不是泛雅平台的替代品，也不是独立运行的通用聊天机器人：它通过外部接口适配接入课程上下文，在内部完成材料解析、证据治理、课程生成、知识包构建、智能体协作、媒体发布与学习证据处理。
...
关键机制：LangGraph 显式工作流（节点边界即权限边界，TeachingAgent 22 节点）、per-tool policy check（`_governance_check` 在每个工具节点前执行，HIGH_RISK_TOOLS 治理异常时 fail-closed 默认禁用）、`ScopeValidator` 强制课程/学生/成员/能力校验。
...
│   ├── SmartCarb项目技术文档（参赛终稿）.md  # A12 赛题参赛技术文档（2026-08-20 更新入库）
...
| [docs/SmartCarb项目技术文档（参赛终稿）.md](docs/SmartCarb项目技术文档（参赛终稿）.md) | A12 赛题参赛技术文档（三项核心机制、五层架构、部署与验证、RE-KT 研究） |
...
> 文档更新清单、挑战杯文档规划方案、XH202620_文档规划）不再有引用。
...
- **2026-08-20**：README 依据《SmartCarb 项目技术文档（参赛终稿）》更新：项目定位统一为"让课程回应学习：知识图谱驱动的可追溯智能教学系统"，新增三项核心机制、五层架构、多智能体协作、AI 安全围栏、云端部署与验证状态章节；参赛技术文档终稿归档至 `docs/SmartCarb项目技术文档（参赛终稿）.md`。
...
*本 README 依据《SmartCarb 项目技术文档（参赛终稿）》（2026-08-20 归档于 `docs/`）更新；所有功能状态均可回溯至代码证据。*
