...
> **CS 语料检索路线（2026-09-08，NEXT）：**[向量化、索引与 RAG 上线实施计划（CR0–CR6）](docs/phase1/2026-09-08_CS语料向量化与RAG上线实施计划.md)采用“原文切块 + 本地多语言 embedding + 全文/向量混合检索”，统一接入学科页面、TeachingAgent 与 Nexus AI；取消全量 LLM 概念/关系抽取。[旧 DK 方案已替代](docs/phase1/2026-09-08_学科知识库自动化构建实施计划.md)：DK1/DK4 修改后复用，DK2 未实现且取消，DK3 退出本路线。单编码智能体预计 33–52 有效工时，全量计算另计；现有 DK 局部测试复验 65 passed，不代表新 RAG 功能已实现或上线。
...
>
> **2026-09-03 CodeNexus 转型启动（Nexus AI）**：`feature/xh202620` 全部提交已合并回
> `dev-liu`（merge `67f94026`，此后单一主线开发）。产品收敛为双入口——
> **TeachingAgent**（课程内便捷问答智能：教学问答、对话式代码挑战）与
> **Nexus AI**（课程外全局入口，建设中：复杂问题拆解、论文研究、快速复现，基于
> Deep Agents + LangGraph 独立 Runtime）。旧 ResearchAgent 科研工作台转 Legacy，
> 前端四面板将随 Nexus 上线下线；SmartCarb 时代文档已清理删除。服务实际部署于
> `http://47.99.97.154/`。决策全文见
> [docs/phase1/2026-09-03\_CodeNexus转型实施决策.md](docs/phase1/2026-09-03_CodeNexus转型实施决策.md)。
