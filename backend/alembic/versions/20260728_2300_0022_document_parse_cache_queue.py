...
def upgrade():
    ...
    existing_columns = {column["name"] for column in inspector.get_columns("document_ir_versions")}
    if "parser_profile" not in existing_columns:
    ...
    if "ix_document_ir_versions_cache_key" not in {index["name"] for index in inspector.get_indexes("document_ir_versions")}:
        op.create_index("ix_document_ir_versions_cache_key", "document_ir_versions", ["cache_key"])
    if not inspector.has_table("document_parse_owner_leases"):
        op.create_table(
