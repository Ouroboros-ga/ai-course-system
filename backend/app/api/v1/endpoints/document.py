...
async def create_course_import(
    ...
    """Create a draft course and persist its parsing task before returning.

    This is the creation path used by the new UI.  It deliberately does not call
    the legacy synchronous document-to-script flow and never auto-publishes a
    course.  Once this endpoint returns 202, the source object and TaskRecord
    are durable: leaving the browser cannot cancel parsing.
    """
