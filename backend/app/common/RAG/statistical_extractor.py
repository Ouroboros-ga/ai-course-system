...
使用统计方法从文本中提取专业名词，包括：
1. 互信息 (Mutual Information) - 衡量字之间的关联强度
2. 左右熵 (Left/Right Entropy) - 衡量词边界的稳定性
3. TF-IDF - 衡量词的重要性
4. C-value - 衡量嵌套术语的重要性
...
class StopWordsFilter:
    ...
    def is_stopword(cls, term: str) -> Tuple[bool, str]:
        ...
        for pattern in cls.NON_TERM_PATTERNS:
...
class StatisticalTermExtractor:
    ...
    def _generate_candidates(self, texts: List[str]) -> None:
        ...
        for text in texts:
            ...
            for segment in cn_segments:
                for length in range(2, min(self.max_term_len + 1, len(segment) + 1)):
    ...
    def _apply_filters(self) -> List[TermCandidate]:
        ...
        for term, candidate in self._term_candidates.items():
            is_stop, reason = StopWordsFilter.is_stopword(term)
            if is_stop:
