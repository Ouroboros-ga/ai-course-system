...
class Doc:
    ...
    def save_as_md(self, output_path: Optional[Union[str, Path]] = None) -> Markdown:
        """
        保存为 Markdown 文件
        ...
        if output_path is None:
            resolved_output_path = self.file_path.with_suffix(".md")
        else:
    ...
    def save_as_json(
        ...
        resolved_output_path: Path
        if output_path is None:
            resolved_output_path = self.file_path.with_suffix(".json")
        else:
            resolved_output_path = Path(output_path)
...
class DocProcessor:
    ...
    def parse_file(self, file_path: Union[str, Path]) -> Doc:
        """
        解析单个文件并返回 Doc 对象
