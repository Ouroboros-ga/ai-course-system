...
class TestMediaTimelineAPI:
    ...
    def test_dh_presets(self, client, session):
        """数字人预设列表"""
        teacher = _user(session, "mt_api_dh")
        _setup(session, teacher)
        token = _token(teacher)
        response = client.get(
            "/api/v1/media/digital-human/presets",
            headers={"Authorization": f"Bearer {token}"},
        )
        assert response.status_code == 200
        data = response.json()["data"]
        presets = data["presets"]
        assert any(p["preset"] == "dh_live_mini" for p in presets)
        assert any(p["preset"] == "lite_avatar" for p in presets)
