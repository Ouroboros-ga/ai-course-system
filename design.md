...
> **版本**：v0.6
> **作品定位**：面向计算机学科的垂类大模型与可执行智能体平台（首页与全部对外材料统一使用此句；白话阐释为"让知识有出处，让代码跑得通，让学习有回响"）
> **教学侧主张**：让课程回应学习（课程侧沿用，不因对外定位变更而废弃）
> **叙事口径源**：[`docs/phase1/2026-09-09_CodeNexus_作品定位与叙事口径.md`](./docs/phase1/2026-09-09_CodeNexus_作品定位与叙事口径.md)
...
| 助教智能体面板（`--agent-panel-width`）    | 右上角工具球折叠；展开为 460px 布局挤压列 |                          360px |
...
## 7.2 布局形态：右上角工具球 + 布局挤压式面板

- **折叠态**：舞台区（`.build-stage`）右上角 46px 圆形工具球（`.agent-fab`，Sparkles 图标，`position:absolute; top/right: var(--space-3)`，`box-shadow: var(--shadow-md)`），悬停出现外圈光晕。舞台标题区（`.stage-context`）右侧预留 52px 防止操作按钮被球遮挡。

- **展开态**：`.agent-dock` 作为 `.build-grid` 的 flex 布局列（rail 与舞台之后），**直接挤压舞台区宽度**（非浮层覆盖）。dock 宽度 `0 → var(--agent-panel-width)` 约 200ms 过渡；内部面板定宽，避免动画期间内容被压扁。收起时面板 `visibility: hidden` 防止 Tab 聚焦进隐藏区域。

- **收回入口**：面板头部关闭按钮（`AgentHeader.close-agent`，任何断点均显示）→ `agentOpen = false`，dock 收回 0 宽、工具球重新淡入；移动端顶部 `mobile-workbench-tabs` 亦可切换。

- 默认宽度：`--agent-panel-width: 460px`；≤760px 移动端 dock 全宽出现在舞台下方（高度过渡，`min(70vh, 640px)`）。
...
- 助教智能体面板为舞台区右上角工具球折叠 + 布局挤压式展开（§7.2），不随 1250px 断点隐藏；工具球与面板在所有断点可用
...
- 进入时暂停媒体并冻结 return anchor，退出后恢复到精确播放位置；刷新恢复当前 offer/session，不串接旧 run\_id
...
- CourseLayout 二级导航返回按钮（`.sfx-l2nav > div > div > button`）的目标必须按当前路由路径判断：
  ...
  - 当前在 `/build/*` 下（含 `/build/knowledge/*`）→ 教师回到 `/app/courses/building`（"我建设的"），学生回到 `/app/courses/learning`（"我学习的"）

  - 其他子页面（overview/learn/experiments/members/settings）→ 回到 `/app/courses/learning`（"我学习的"）
...
无 `course.edit` 的角色进入 `/build/*` 建设布局时，**整个建设侧栏与收起按钮不再渲染**（2026-09-07 需求：学生仅剩「结构视图」一个页面，单项导航没有存在意义），舞台全屏呈现结构视图；九步建设导航、知识子菜单、建设提示语、助教智能体面板/工具球与 STEP 编号同样不出现，避免学生看到无权限的建设页面。舞台顶部标题栏（`.stage-context`，含页面标题与动作按钮）整块也不再渲染，学生端不再出现「结构视图」占位标题（2026-09-11 需求）。
