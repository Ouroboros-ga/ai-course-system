...
- [SQLite 到独立 PostgreSQL 的迁移与服务器切换](phase1/2026-08-11_SQLite到PostgreSQL迁移与服务器切换.md)：当前 PostgreSQL 兼容修复、可审计 SQLite 快照迁移、隔离预演、维护窗口切换、回滚边界和备份恢复入口。服务器实际部署文件为 `deploy/postgres/`；正式数据尚未切换前，不得将本文表述为已上线。
