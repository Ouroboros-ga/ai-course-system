...
> **2026-09-09 Nexus 功能补齐（现行实施入口，F1–F9 已实施并上线验证，F9 收尾中）**：[原设计功能补齐与开源集成实施计划](phase1/2026-09-09_Nexus功能补齐与开源集成实施计划.md)。依据当前代码与原始产品要求，订正“主体完成、仅 5 项缺口”口径；F1–F9 补齐判定、持久恢复、操作中断/资源/Console、自动准备与 repo2docker、冻结与干净 B、正式多格式成果、完整研究及受控 A/B。明确沿用 Deep Agents/LangGraph/SWE-ReX，新增核心首选 repo2docker/Pandoc，禁止自建通用调度/PTY/格式转换器。实施与线上验证见该文件各批注记；已知缺口（配方链后端断裂待发、对照工作台展示、对照层预算/批准执行力、自主路径两项线上实证、组合流模型钥匙）见 2026-09-10 审计。下方较早的状态描述按时点保留，Nexus 补齐范围以此入口为准。

> **正式对外文档（2026-09-09 整理）**：
> - 项目总览：[`../README.md`](../README.md)（重写为正式项目文档，剥离开发日志、剔除失效旧地址，能力口径与规划/进行中能力分开）
> - 参赛技术报告：[`../competition/05-作品代码/技术报告.md`](../competition/05-作品代码/技术报告.md)（v2.0，2026-09-09 改为创新主线组织：八项关键技术创新 + 架构概览，代码索引齐全）
> - 安全审计与参赛材料打包核查：[`../docs/安全审计与参赛材料打包核查.md`](../docs/安全审计与参赛材料打包核查.md)（2026-09-09：HEAD 树干净；Git 历史含旧 `.env` 真实形态凭据，打包需剔除 `.git`）
> - 旧 README 的开发日志/时间线迁移为 Git 历史追溯，不再作为现状证据；能力口径以本索引 §"先读什么" 与 `AGENTS.md` 为准。

> **2026-09-08 CS 语料 RAG（NEXT，现行入口）：**[向量化与 RAG 上线实施计划](phase1/2026-09-08_CS语料向量化与RAG上线实施计划.md)，覆盖 CR0–CR6 语料盘点、持久原文/切块、本地 embedding、FTS/pgvector、版本发布、TeachingAgent/Nexus/页面接线、迁移部署与回退；含 DK 复用审计、验收门及 33–52 有效工时估算。CR0–CR6 代码与部署工具已落地于 dev-liu 工作区（**未提交、未部署**），经全量审核与修复（见[全量审核记录](phase1/验收记录/CS语料RAG全量审核_2026-09-08.md)、[修复计划](phase1/验收记录/CS语料RAG修复计划_2026-09-08.md)）；真实模型、语料、部署与登录验收仍未执行。
>
> **已替代 / 仅历史：**[DK0–DK8 自动抽取方案](phase1/2026-09-08_学科知识库自动化构建实施计划.md)。DK1/DK4 修改后复用；DK2 未实现并取消；DK3 核验/裁决退出本路线。65 项旧 DK 测试通过不等于抽取流水线完成。证据与边界见[功能现状审计表](phase1/功能现状审计表.md)。
...
> **Nexus 下一主线（基线 a60d4e87）**：[持续研究V1任务书](phase1/Nexus_持续研究V1_开源集成与开发任务书.md)为当前下一批排期及用户结论的重要设计参照，§1.1明确一次确认后沙箱内自主配置/运行/排错，§11将自主实验纵向链优先，完整研究不作实验前置审批；含开源采用边界和正式输出SR6；[研究实验与成果总路线](phase1/Nexus_论文到实验_下一阶段实施计划.md)保留N0–N4与最终验收规格；[P2 §10](phase1/CodeNexus_P2开发计划.md#10-当前完成度与下一主线本次冻结)区分历史 MVP、AB/CD/v6 实现状态和未完成核心链。新计划状态为 NEXT，文档更新不代表功能已实现。

> **2026-09-09 新增:XH-202620 微调交付(7B LoRA · 数据 v2.2)**——
> [phase1/2026-09-09_XH202620微调交付记录.md](phase1/2026-09-09_XH202620微调交付记录.md)
> 与交付对照表 `backend/finetune/export/微调交付_模型与对照表.md`(adapter 模型文件已产出,
> 训练可复现 train_loss 0.667–0.669;基准 10 问评测**如实记录为基座 4/8 vs 微调 3/8,不宣称提升**;
> ServiceID 待星火 MaaS)。材料 05 清单已回填模型文件栏。
...
> evidence episode。学生界面不再区分 CodingAgent，也不区分运行/正式提交；旧
> `/experiments` 与独立 coding runtime 仅兼容。代码与组件级浏览器验收已完成，真实课程
> Judge0 完整冒烟、PostgreSQL 迁移演练和部署仍未执行。
...
> 达 90 节点/82 关系/9 门课；微调指令集 164+18；知识库测试 17 passed；全量后端回归复跑
...
> 2026-08-30 更新（XH-202620 R13）：学科知识库扩充离散数学（12 节点，`discrete.json`）+
> 计算机图形学（10 节点，`graphics.json`）及 24 条新关系（含跨课 prerequisite/uses），
> 现 112 节点/106 关系/11 门课；`validate.py` 通过、知识库测试 20 passed（新增 3 个检索/
> 概览验证）；README、竞赛材料 03/04/05/07 与本索引已同步。

> 2026-08-31 新增：**证据生成链路审计与改进方案**——[phase1/2026-08-31_证据生成链路审计与改进方案.md](phase1/2026-08-31_证据生成链路审计与改进方案.md)：
> 全库 8 条活跃证据链路审计（教师确认漏斗转化率 1.5%、噪声过滤覆盖 1/6 课程、
> 重解析不退役旧候选、图谱候选无噪声门），含外部研究引用与 P0-P3 改进提案。
> 其中 **§8 布局解析改进已实施**：`providers/pdf_layout.py` 框感知布局分析
> （Union-Find 文本框聚类、相对标题判定、代码区聚合、CJK 字空格容差、
> 公式残片吸收、符号残片过滤）替换 pdf-plumber 行级提取；projector 新增
> `code`→`code_example` 映射；真实课程 PDF 验证 u1 教材数学残片 -92%、
> 字空格标题恢复完整；测试 15+240+16 通过（256 passed）。基线同步见
> [统一课程建设与解析基线](phase1/统一课程建设与解析基线.md) §4.2。
> 服务器旧 IR 需重新解析并由教师显式 `/apply` 才切换；解析成功不会自动采用。

> 2026-08-31 XH 本地加固：P0 的统一原因编码噪声门已接入 projector、新旧图谱
> 候选教师视图、EvidenceSpan 列表和确认入口；默认审核列表绑定 active IR，显式历史查看保留，
> STALE 与无教育意义碎片不能确认。远端存量治理、批量确认和部署仍未执行。
...
2. [对话式代码挑战实施说明](phase1/2026-09-01_对话式代码挑战实施说明.md)：学生体验、TeachingAgent 内部代码能力、API、迁移、episode 证据与未完成验收。
