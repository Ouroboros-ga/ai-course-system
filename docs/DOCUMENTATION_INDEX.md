...
> 2026-08-09 更新：首轮智能备课的材料证据整理已从单次 LLM 请求升级为有界 Map/Reduce；同日真实复测发现 Reduce 的 `examples`/`exercises` 可在修复重试后仍超过 schema 上限，现已增加 Reduce 专用稳定去重与十项裁剪。资源上限、证据追溯、失败原因与验证数据登记在[统一课程建设与解析基线](phase1/统一课程建设与解析基线.md)和[功能现状审计表](phase1/功能现状审计表.md)。公开课程建设路由不变，无数据库迁移。
