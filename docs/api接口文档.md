...
根据课程节点内容，使用AI生成一道检验理解的单项选择题。题目包含4个选项、正确答案和详细解析。
...
| **参数名**   | **类型** | **必填** | **说明**                                      |
|------------|--------|--------|---------------------------------------------|
| courseId   | Integer | 否      | 课程ID，传入后会附加课程上下文增强出题质量              |
| nodeId     | Integer | 否      | 节点ID，传入后自动从数据库获取节点内容和标题             |
| nodeContent | String | 否      | 节点内容（若不传nodeId则使用此内容）                  |
| nodeTitle  | String  | 否      | 节点标题，默认为空                                |
...
```json
{
  ...
  "data": {
    "quiz": {
      ...
      "correct_answer": "B",
      "explanation": "电力电子是电气工程的重要分支，专注于电能的转换和控制，如电动汽车电机控制、太阳能板能量转换等。选项A错误，电气工程涵盖发电、输电、配电、用电全环节；选项C错误，新能源是电气工程的重要应用领域；选项D错误，电力系统负责从发电厂到终端用户的整个输配电过程。"
    }
  }
}
```
