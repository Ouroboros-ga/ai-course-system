...
> **当前进度（2026-09-05 纠偏）**：S0–S3、P1 与 M0–M5 的 MVP 验收见 §九和 P2 计划历史记录；不等于完整 Harness/论文研究/复现平台完成。后续必要主线统一见 [NX 开发计划](CodeNexus_P2开发计划.md)：八格式附件与图片直传、Todo/Subagent、Paper Research、成熟 SandboxProvider、Paper-to-Reproduction/A-B 验证、Experiment Console、服务端 Session。原 S3 任务描述保留作迁移前规划，实际删除/保留清单以 §九纠正记录为准，不重做已验收迁移。
...
**计划状态（2026-09-05）**：S1 双轨期与 S3 下线不再作为下一步。后续按 NX 开发计划 的 NX-A/H/R/S/P 与 NX-E1–E4 推进；保留稳定 preset 演示，逐项验收。任何部署、依赖安装仍遵守 AGENTS.md，不因规划更新而自动获准。
