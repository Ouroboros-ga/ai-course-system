...
> **版本**：v2.3 (2026-09-05，产品方向纠偏)
> **阶段**：M0–M5 已验收 MVP；真实对话、检索、Artifact 与预设复现已接线。附件、完整 Harness/Paper Research、A/B 实验、实时 Console 和跨设备执行历史仍需建设。原 S1“纯前端演示”段落仅作历史，不再作为当前状态依据。
...
> 下表为 S1 初始阶段快照，已由 M0–M5 验收及本文件 §14 的现行开发方向取代，不可据此关闭已上线功能或删除后续产品目标。
...
> **2026-09-05 勘误**：此前“全包无 todo/planning 中间件”的判断不成立。默认工具面未包含 Todo 不代表能力被删除，可显式接入 TodoListMiddleware；M1 删除不匹配的 Prompt 规则只修复了当时的不一致。最终产品按复杂度恢复 Todo/Planning，前端消费真实事件，不用静态清单伪装任务进展。
...
## 14. 2026-09-05 已确认后续产品开发方向（待实施）

本节纠正早期“纯演示”“仅 PDF”“工具过程不需恢复”等范围假设，现行任务与依赖见 [P2 开发计划 §11.1](CodeNexus_P2开发计划.md)。当前 M0–M5 完成仅代表预设 MVP；以下按独立能力验收，不提前改 ready。视觉实现继续遵守根目录 design.md。

| 前端工作项 | 产品行为与接线要求 | 验收边界 |
| --- | --- | --- |
| 八格式附件（NX-A1） | General/Research 共用上传入口；PDF/DOCX/JPG/PNG/XLSX/PPTX/PPT/DOC 文件卡含状态、移除/重试、可读范围；请求传 attachment_ids，引用可定位页/slide/sheet-cell/段落 | 上传不等于可读；owner/session 服务端验证；过期与删除可见，不自动进课程知识域 |
| 图片直传（NX-A1） | 缩略图+用户问题进入视觉模型消息；无需等待 OCR；不支持视觉时明确提示或使用已配置视觉辅助 | 前后端完整传递图片块；OCR 仅为文字降级，不假称已理解图表 |
| Plan/Todo/Subagent（NX-H1/H2） | Research 长任务显示真实计划/修改/步骤状态与受限子任务；复杂 General 按需；简单任务不强制清单 | 消费真实产品事件、支持恢复与取消；不显示模型内部思维，不开放宿主任意 shell |
| Paper Research（NX-R1） | 展示论文来源、全文可用性、证据、方法比较与可核查 Citation | 只有 arXiv 候选列表时标 Paper Search；无法读取全文不能伪造阅读结果 |
| 论文复现（NX-P1/P2） | Paper+Repo→可执行 Claim/计划→环境/运行→冻结验证→指标与报告；保留 nanoGPT preset 入口 | 计划不等于已运行；普通执行 PASS 不等于干净环境 B 验证成功 |
| Experiment Console（NX-E2/E3） | 会话内基础卡展开只读详情：Stage、Command label、Elapsed、Exit code、20 行脱敏日志、Metric、Report、Cancel | Worker 增量输出先接线；预构建/未实现 B 如实显示；取消须 Worker 确认，不是停止聊天；不做交互终端 |
| Session / Execution History（NX-E1/E4） | 服务端 mode/course/pin 与 run/job/附件/产物关联；刷新及换设备恢复快照，再按游标加载最小事件、恢复 job 轮询 | localStorage 仅缓存；事件去重、按用户隔离；不重复提交实验，不由回答补造 Trace；checkpoint 与产品历史分开 |

前端顺序：附件生命周期和 run/job 恢复基础 → 图片/各格式可读接线与 Console → Cancel → 元数据/事件跨设备恢复 → Todo/Subagent、Paper Research、A/B 实验投影（按对应后端能力到位）。每项记录真实接口/页面证据；本次只修订规格，不实施 UI。
