...
**推荐链路 PostgreSQL 兼容修复（2026-08-18）**：`question_bank_items.knowledge_node_ids` 为 `Column(JSON)`（PostgreSQL `json` 类型），此前 5 处查询使用 `knowledge_node_ids.contains([node_id])`，SQLAlchemy 对其编译为 `LIKE`，PostgreSQL 16 报 `operator does not exist: json ~~ text`，导致 `generate_recommendation` 选题目阶段直接抛异常、推荐记录永不落库，`weak_concepts` 恒为空、`prerequisite_review` 教学动作与跳转推荐在问答链路中不可达（本地 SQLite 为 TEXT 列故测试不暴露）。修复：新增 `app/core/db_json.py::json_array_contains`（cast 为 TEXT 后 LIKE，双数据库兼容），替换 `recommendation_service`、`practice_recommendation_service`、`question_bank.py`、`facade.py`、`research/question_bank.py` 共 5 处调用。服务器实测课程4学生200推荐正常产出并持久化（practice_quiz/diagnostic/continue 等），薄弱前置检测按数据阈值正常生效。同日补充前端：放弃按钮不再受 `learningAdjustmentBusy` 限制（busy 卡死也必须能退出），并新增 `adjustmentAbandoned` 标记阻止 apply 挂起回调在放弃后重新激活绿色框。

**概念解析修复：resource_id 优先 + outline 标题回退（2026-08-18）**：`resolve_concepts` 此前完全忽略浏览器传入的 `resource_id`（学生当前学习位置），纯文本/语义检索失败时会把"请问这里涉及到的公式是什么？"解析到源文本切分产生的单字噪声节点（如标题为"的"的图谱节点），导致智能体不知道学生正在学习的知识点，也无法给出前置知识跳转。修复三层：① `resource_id`（`on_*` outline id / `kn_*` 图节点 key / 数字 id）优先解析为当前学习位置；outline 无 `knowledge_graph_node_id` 映射时（课程5 数据缺口）用标题共享关键词回退定位图谱节点。② 点名匹配要求标题 ≥2 字符（排除"的"等单字噪声）且完整出现在问题中；学生明确点名时以点名优先，否则默认停留在当前学习位置。③ 候选名匹配改为子串命中（学生说"传递函数"，图谱标题"一、 传递函数的定义和主要性质"应命中），回退语义检索同样过滤单字标题。回归测试 `tests/agents/test_active_bundle_resolve_concepts.py` 覆盖 resource 优先、点名优先、标题回退、候选子串匹配四类场景。
