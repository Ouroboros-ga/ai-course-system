...
> 状态：现行实施基线（2026-08-12，Harness v1）
>
...
| [pgvector](https://github.com/pgvector/pgvector) | PostgreSQL vector 类型与 cosine distance | 已验证 PostgreSQL 16.14 + pgvector 0.7.4 使用 `VECTOR` 和 `<=>`，按 embedding 维度过滤；保持 PostgreSQL 18 前向兼容 | 模型未冻结时不创建错误维度的 HNSW 索引 |
...
│   ├── workflow.py       # 条件节点与工具分支的 LangGraph
...
`create_all`。真实部署 PostgreSQL 16.14 + pgvector 0.7.4（PostgreSQL 18 仍前向兼容）先启用 pgvector，SQLite 只承担本地 Demo/迁移测试兼容：
...
- 持久化 Todo/Notepad/Scope/Memory，已验证 PostgreSQL 16.14 + pgvector 0.7.4，并保持 PostgreSQL 18 前向兼容；
...
- 真实部署的 PostgreSQL 16.14 + pgvector 0.7.4 已只读验证 `vector` 类型、`<=>` 余弦运算符和 `0053` 的五张工作区表。Provider 不修改服务配置；若 pgvector SQL 执行失败，只回滚当前失败事务并降级关键词检索。
