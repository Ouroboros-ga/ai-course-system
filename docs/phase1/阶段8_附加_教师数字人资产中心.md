...
> **已废弃为现行平台角色说明（2026-08-10）**：本文仅保留“教师授权形象资产”后续扩展的历史设计，不是当前媒体主链的验收依据。当前新建设默认使用 `platform-female-instructor-v1@1.0.0`（`portrait_patch_v1`、课程/Release 签名纹理）；`platform-instructor-real-v1@1.0.0` 汽车教师已退休，只为历史冻结 Release 兼容解析。以 [阶段8媒体实施规划](阶段8_媒体TTS数字人PPT_实施规划.md) 为准。

> **路线状态（2026-08-06）**：本文描述的是后续“教师授权资产接入”扩展，不是当前
> 首版媒体/数字人验收主链。当前课程媒体固定使用平台预制 PixiJS 2D 角色
> 平台角色由 P5.1 注册表提供（当前默认 `platform-instructor-real-v1@1.0.0`，半写实虚构汽车教师）；学习端以发布 `audio-playlist/v1` 的音频时钟驱动角色。
...
平台注册角色（当前默认 `platform-instructor-real-v1@1.0.0`，半写实虚构汽车教师）+ `avatar-cues/v1`；课程 87 的验收先走 Fake WAV、
