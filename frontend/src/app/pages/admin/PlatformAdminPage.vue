...
import { useSettingsStore } from '@/stores/userSettings' // 根据实际路径调整
    ...
    <div class="sfx-page admin-page">
        ...
        <template v-else>
            ...
            <section class="sfx-panel admin-section">
                <div class="section-head">
                    <h2 class="sfx-t-title3">
                        <SlidersHorizontal :size="19" /> Provider 配置
                    </h2><span class="sfx-t-caption">密钥仅显示配置状态；留空保存会保留旧密钥。</span>
                </div>
                <div class="provider-grid">
                    <article v-for="item in integrations" :key="item.integration_key" class="provider-card">
                        <header>
                            <h3>{{ item.integration_key.toUpperCase() }}<span
                                    v-if="['llm', 'tts', 'asr'].includes(item.integration_key)"
                                    class="toggle-caption">{{
                                        item.enabled ? '真实接入' : '已关闭' }}</span></h3>
                            <span class="card-actions"><span class="health" :data-status="item.health_status">{{
                                item.health_status
                                || 'not_configured' }}</span>
                                <SfxButton v-if="['llm', 'tts', 'asr'].includes(item.integration_key)" size="sm"
                                    :variant="item.enabled ? 'secondary' : 'primary'"
                                    :loading="saving === `toggle-${item.integration_key}`"
                                    @click="toggleIntegration(item)">{{
                                        item.enabled ? '关闭真实接入' : '开启真实接入' }}</SfxButton>
                            </span>
                        </header>
                        <label>Provider<input v-model="drafts[item.integration_key].provider" class="sfx-input"
                                placeholder="openai / volcengine / xfyun" /></label>
                        <label>Base URL<input v-model="drafts[item.integration_key].base_url" class="sfx-input"
                                placeholder="https://…" /></label>
                        <label>Model Name<input v-model="drafts[item.integration_key].model_name" class="sfx-input"
                                placeholder="模型或端点名称" /></label>
                        <label>API Key<input v-model="drafts[item.integration_key].api_key" class="sfx-input"
                                type="password"
                                :placeholder="item.key_configured ? `已配置（末四位 ${item.key_last4 || '****'}）` : '输入新密钥'" /></label>
                        <label class="checkbox-line"><input v-model="drafts[item.integration_key].enabled"
                                type="checkbox" /> 保存后启用该
                            Provider</label>
                        <details>
                            <summary>高级配置（JSON）</summary><textarea v-model="drafts[item.integration_key].extra_config"
                                class="sfx-input json-input" rows="4" />
                        </details>
                        <footer>
                            <SfxButton size="sm" :loading="saving === `integration-${item.integration_key}`"
                                @click="saveIntegration(item)">保存并热刷新</SfxButton>
                            <SfxButton size="sm" variant="secondary"
                                :loading="saving === `probe-${item.integration_key}`" @click="probe(item)">测试
                            </SfxButton>
                        </footer>
                    </article>
                </div>
...
.health[data-status="configured"] {
    background: var(--green-100);
    color: var(--green-700);
}

.health[data-status="unavailable"],
.health[data-status="not_configured"] {
