      ...
      <main class="media-main">
        ...
        <section class="release-panel batch-panel" aria-labelledby="batch-title">
          ...
          <div class="tts-actions">
            ...
            <SfxButton variant="secondary" size="sm" :disabled="!canPlanBatch" :loading="acting === 'batch-plan'" @click="createBatchPlan">核算批量费用</SfxButton>
            ...
            <SfxButton size="sm" :disabled="!canConfirmBatch" :loading="acting === 'batch-confirm'" @click="confirmBatch" title="一次提交所选全部知识点的语音合成，并自动冻结字幕与数字人时间轴；无需在下方重复手动提交">生成全部所选知识点语音</SfxButton>
          </div>
        ...
        <template v-if="selectedScript">
          ...
          <section class="release-panel" aria-labelledby="release-title">
            ...
            <div v-if="!workingRelease" class="release-empty">
              ...
              <SfxButton size="sm" :disabled="!canCreateDraft" :loading="acting === 'create-release'" @click="createDraft">创建媒体验收草稿</SfxButton>
            </div>
            ...
            <template v-else>
              ...
              <div v-else class="workflow-list">
                <article class="workflow-row" :class="{ complete: Boolean(selectedTtsJob?.status === 'succeeded'), active: !selectedTtsJob }">
                  ...
                  <div class="workflow-copy"><span>01 · 语音合成</span><strong>将当前讲稿提交给已配置的服务器端音色</strong><p v-if="selectedTtsJob">{{ jobStatusLabel(selectedTtsJob) }} · {{ selectedTtsJob.output_metadata?.cache_hit ? '命中已有音频缓存' : selectedTtsJob.error_message_safe || '任务状态会自动刷新' }}</p><p v-else-if="isPlaylistRelease">该知识点不在本批已确认的节点内；请在上方批量面板勾选后重新核算并确认。</p><p v-else>提交前需教师明确确认；页面不会读取或展示任何密钥、音色 ID。</p></div>
                  <SfxBadge v-if="selectedTtsJob" :tone="jobTone(selectedTtsJob)">{{ jobStatusLabel(selectedTtsJob) }}</SfxBadge>
                </article>

                <div v-if="!isPlaylistRelease && (!selectedTtsJob || selectedTtsJob.status === 'failed')" class="tts-confirmation">
                  ...
                  <div class="tts-actions">
                    <SfxButton v-if="selectedTtsJob?.status === 'failed'" size="sm" :disabled="!canSubmitTts" :loading="acting === 'retry-tts'" @click="retryTts">确认并重试一次</SfxButton>
                    <SfxButton v-else size="sm" :disabled="!canSubmitTts" :loading="acting === 'submit-tts'" @click="submitTts"><Send :size="14" /> 为当前讲稿提交语音（单知识点）</SfxButton>
                ...
                <div v-if="!isPlaylistRelease && selectedTtsJob?.status === 'succeeded' && !hasFrozenCues && (!cueJob || cueJob.status === 'failed')" class="workflow-action"><SfxButton size="sm" :disabled="!canGenerate" :loading="acting === 'freeze-cues'" @click="freezeCues">冻结字幕与数字人时间轴</SfxButton></div>
                ...
                <div v-if="canBuildPptManifest && !hasPptManifest" class="workflow-action"><SfxButton v-if="!pptManifestInFlight" variant="secondary" size="sm" :disabled="!canGenerate || acting === 'ppt-manifest'" @click="createPptManifest">{{ pptManifestJob?.status === 'failed' ? '重试 PPT manifest' : '生成 PPT manifest' }}</SfxButton><small v-else>后台任务执行中；本页面每 5 秒刷新一次进度，无需保持本次请求。</small></div>
                <div v-if="isPlaylistRelease" class="workflow-action">
                  <p v-if="hasFrozenPlaylist" class="task-output">课程播放清单已固定到此版本，后续不会自动更改。</p>
                  <SfxButton v-else variant="secondary" size="sm" :disabled="!canGenerate || !hasPptManifest" :loading="acting === 'batch-freeze'" @click="freezeBatchPlaylist">冻结课程播放清单</SfxButton>
                ...
                <div v-if="workingRelease.status === 'draft'" class="workflow-action"><SfxButton size="sm" :disabled="!canPublish || !canActivateWorkingRelease" :loading="acting === 'activate'" @click="activateRelease">激活媒体版本</SfxButton></div>
                <div v-else-if="workingRelease.status === 'active'" class="workflow-action"><SfxButton variant="secondary" size="sm" @click="goToRelease">前往正式发布</SfxButton></div>
              </div>
            </template>
          </section>
...
<style scoped>
.batch-panel{border:1px solid var(--border-strong);background:var(--surface-panel)}.batch-panel .tts-actions{flex-wrap:wrap;margin-top:0;padding:var(--space-3) var(--space-4);border-top:1px solid var(--border-subtle);background:var(--surface-cool)}.batch-panel .tts-actions .sfx-btn{flex:1 1 auto;min-width:max-content}.batch-estimate{display:flex;flex-wrap:wrap;gap:var(--space-4);padding:var(--space-3) var(--space-4);color:var(--text-secondary);font-size:var(--ui-sm-size)}.batch-estimate p{flex-basis:100%;margin:0}.batch-flow-hint{margin:0;padding:var(--space-2) var(--space-4) var(--space-3);color:var(--text-muted);font-size:var(--caption-size);line-height:1.5;border-top:1px solid var(--border-subtle)}.batch-status{display:flex;flex-wrap:wrap;gap:var(--space-3);padding:var(--space-2) var(--space-4);border-top:1px solid var(--border-subtle);color:var(--text-secondary);font-size:var(--caption-size)}.batch-item-list{display:grid;border-top:1px solid var(--border-subtle)}.batch-item-row{display:flex;align-items:center;justify-content:space-between;gap:var(--space-3);padding:var(--space-2) var(--space-4);border-bottom:1px solid var(--border-subtle)}.batch-item-row>div{display:grid;gap:2px;min-width:0}.batch-item-row strong{color:var(--text-primary);font-size:var(--ui-sm-size)}.batch-item-row small{color:var(--text-secondary);font-size:var(--caption-size);overflow-wrap:anywhere}
...
.script-item-row{position:relative;display:flex;align-items:center;gap:var(--space-2);min-height:54px;padding:var(--space-2) var(--space-2) var(--space-2) var(--space-3);border:1px solid transparent;border-radius:var(--radius-md);cursor:pointer;transition:background var(--duration-fast) var(--ease-out)}
.script-item-row:hover{background:var(--surface-cool)}
.script-item-row.selected{background:var(--ink-100);color:var(--ink-900)}
.script-item-row.selected::before{position:absolute;left:0;top:var(--space-2);bottom:var(--space-2);width:3px;background:var(--ink-900);content:"";border-radius:var(--radius-full)}
...
.script-item-copy small{font-size:var(--caption-size);opacity:.8;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
.media-main{display:flex;flex-direction:column;gap:var(--space-3);min-width:0;min-height:0;overflow-y:auto;padding:var(--space-3) var(--space-6)}.provider-runtime-status{margin:0;color:var(--text-secondary);font-size:var(--caption-size)}.provider-runtime-status.is-blocked{color:var(--red-700)}.provider-demo-note{color:var(--ink-700);font-size:var(--caption-size)}.provider-bar{display:flex;align-items:center;gap:var(--space-2);flex-shrink:0}.provider-bar-name{color:var(--text-muted);font-family:var(--font-mono);font-size:var(--caption-size)}.notice,.action-error{display:flex;align-items:flex-start;gap:var(--space-2);margin:0;padding:var(--space-3);border-radius:var(--radius-md);font-size:var(--ui-sm-size);line-height:1.5;flex-shrink:0}.notice{border:1px solid var(--ink-300);background:var(--ink-100);color:var(--ink-700)}.action-error{border:1px solid var(--red-300);background:var(--red-100);color:var(--red-700)}
