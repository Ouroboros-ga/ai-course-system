...
<template>
  ...
  <section v-else class="sfx-panel"><label class="sfx-t-caption">审核说明（可选）<input v-model="note" class="sfx-input" maxlength="500" /></label><div class="sfx-table-wrap"><table class="sfx-table"><thead><tr><th>申请人</th><th>说明</th><th>状态</th><th>申请时间</th><th>操作</th></tr></thead><tbody><tr v-for="request in requests" :key="request.request_id"><td>{{ request.user_name || request.user_id }}</td><td>{{ request.message || request.reason || '—' }}</td><td><SfxBadge :tone="tone[request.status] || 'neutral'">{{ request.status }}</SfxBadge></td><td>{{ request.created_at ? new Date(request.created_at).toLocaleString('zh-CN') : '—' }}</td><td><span class="sfx-actions"><SfxButton v-if="request.status === 'pending'" size="sm" :loading="working === request.request_id" @click="decide(request, 'approve')">通过</SfxButton><SfxButton v-if="request.status === 'pending'" size="sm" variant="secondary" :loading="working === request.request_id" @click="decide(request, 'info')">补充信息</SfxButton><SfxButton v-if="request.status === 'pending'" size="sm" variant="secondary" :loading="working === request.request_id" @click="decide(request, 'reject')">拒绝</SfxButton></span></td></tr></tbody></table></div></section></div>
</template>
<style scoped>.sfx-joinrequests{display:flex;flex-direction:column;gap:var(--space-4);padding:var(--space-6)}.sfx-joinrequests-head{display:flex;justify-content:space-between;align-items:flex-end;gap:var(--space-3)}.sfx-actions{display:flex;gap:var(--space-2);flex-wrap:wrap}</style>
