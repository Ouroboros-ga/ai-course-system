...
const tools=[['graph_read','图谱检索'],['course_retrieval','课程内容检索'],['question_bank','题库'],['sandbox','代码沙箱'],['visualization','可视化演示'],['learning_event','学习行为记录'],['web_research','外部资料检索']]
...
<template><div class="sfx-agent"><header><h1 class="sfx-t-title2">智能体策略</h1><p class="sfx-t-ui sfx-t-secondary">选择本课程允许智能体使用的能力，并决定高风险动作是否需要教师确认。</p></header><form class="sfx-panel sfx-agent-form" @submit.prevent="save"><label v-for="[tool, toolLabel] in tools" :key="tool" class="sfx-check"><input v-model="form.enabled_tools" type="checkbox" :value="tool" :disabled="!allowed"/><span>{{toolLabel}}</span></label><label class="sfx-check"><input v-model="form.require_teacher_confirmation" type="checkbox" :disabled="!allowed"/>高风险教学动作必须教师确认</label><label class="sfx-check"><input v-model="form.web_research_enabled" type="checkbox" :disabled="!allowed"/>允许补充外部资料（仅供回答参考，不改变课程内容）</label><SfxError v-if="error" :description="error"/><p v-if="saved" class="sfx-save-ok">已保存。</p><SfxButton type="submit" :disabled="!allowed" :loading="saving">保存智能体策略</SfxButton></form></div></template>
