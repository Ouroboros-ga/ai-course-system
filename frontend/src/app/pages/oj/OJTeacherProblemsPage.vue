...
async function load() {
  ...
  error.value = ''
  try {
    const data = await request.get(`/api/v1/experiments/course/${courseId.value}/definitions`)
...
async function changeStatus(item, action) {
  ...
  try {
    await request.post(
      `/api/v1/experiments/course/${courseId.value}/definitions/${item.experiment_id}/${action}`,
    )
    await load()
  } catch (caught) {
