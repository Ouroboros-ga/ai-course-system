<template>
  <article class="task-card" :class="`tone-${meta.tone}`" :aria-label="`${task.title}：${meta.label}`">
    ...
    <div class="task-actions">
      ...
      <button v-if="task.requiresReview && !confirmed" type="button" class="confirm-button" :disabled="loading" @click="$emit('confirm')"><Check :size="15" />教师确认</button>
      <span v-else-if="task.requiresReview && confirmed" class="confirmed-state"><Check :size="15" />本次会话已确认</span>
    </div>
  </article>
</template>
