  ...
  <main class="production-workspace">
    ...
    <div class="workbench-grid">
      ...
      <section class="main-stage" aria-live="polite">
        <div class="stage-heading"><div><p class="eyebrow">当前步骤</p><h2>{{ currentStep.label }}</h2><p>{{ currentStep.description }}</p></div><span class="status-chip" :class="`state-${currentStep.state}`">{{ currentStep.stateLabel }}</span></div>

        <section v-if="activeStep === 'materials'" class="stage-panel">
          <FileUp :size="28" /><h3>沿用已验证的资料上传与解析流程</h3><p>资料上传、Docling 解析和知识结构提取仍由现有课程编辑器负责。本工作台只负责组织进度与质量检查，避免复制另一套上传协议。</p><button type="button" class="primary" @click="goLegacy"><Upload :size="16" />去上传教学资料</button>
        </section>
        ...
        <section v-else-if="activeStep === 'mapping'" class="stage-panel wide-panel">
          <Waypoints :size="28" /><h3>知识点—PPT 映射治理</h3><p>使用已存在的 Mapping API 管理知识点与 PPT 页码范围。Evidence 与知识图谱审核能力尚未接入，此处不会用静态数据伪装为可用功能。</p><button type="button" class="primary" @click="goMapping"><ArrowRight :size="16" />进入映射工作区</button>
        </section>
        <section v-else-if="activeStep === 'ppt'" class="stage-panel wide-panel">
          <Presentation :size="28" /><h3>生成课程 PPT</h3><p>复用现有 PPT 生成对话框。生成完成后回到本工作台进行映射检查。</p><button type="button" class="primary" @click="showPptDialog = true"><Sparkles :size="16" />生成 PPT</button>
          <div class="task-lookup"><label for="ppt-task">已有异步任务 ID</label><div><input id="ppt-task" v-model.trim="pptTaskId" placeholder="输入 sid 后刷新状态" /><button type="button" class="secondary" :disabled="taskLoading || !pptTaskId" @click="refreshPptTask"><RefreshCw :size="16" />查询</button></div></div>
          <LongTaskCard v-if="pptTask" :task="pptTask" :loading="taskLoading" @refresh="refreshPptTask" @retry="openPptRetry" @confirm="confirmTask" />
        </section>
...
import { AlertTriangle, ArrowLeft, ArrowRight, CheckCircle2, CircleDashed, Cloud, ExternalLink, FileCheck2, FileText, FileUp, Info, LoaderCircle, Presentation, RefreshCw, Save, ShieldCheck, Sparkles, Upload, Waypoints, Volume2, Video } from 'lucide-vue-next'
...
import { getPPTTaskStatus } from '@/api/ppt_generation.js'
import LongTaskCard from '@/features/teacher-workspace/components/LongTaskCard.vue'
import { normalizeLongTask } from '@/features/teacher-workspace/taskStatus.js'
import { showToast } from '@/utils/toast.js'

const route = useRoute(); const router = useRouter(); const courseId = computed(() => route.params.courseId)
const activeStep = ref('materials'); const showPptDialog = ref(false); const versions = ref([]); const versionsLoading = ref(false); const versionsError = ref(''); const snapshotLoading = ref(false); const pptTaskId = ref(''); const pptTask = ref(null); const taskLoading = ref(false); const saveState = ref('工作区已就绪')
const steps = [
  ...
  { id:'audio', label:'音频生成', icon:Volume2, state:'pending', stateLabel:'未开始', description:'在脚本确认后生成音频。', legacyTitle:'音频生成', legacyDescription:'批量 TTS 任务仍由现有编辑器发起。' },
  { id:'avatar', label:'数字人生成', icon:Video, state:'pending', stateLabel:'未开始', description:'在音频与映射确认后生成数字人内容。', legacyTitle:'数字人生成', legacyDescription:'数字人生成与资产配置仍在原编辑器。' },
]
const currentStep = computed(() => steps.find(step => step.id === activeStep.value) || steps[0])
const checks = computed(() => [{label:'教学资料已解析',note:'现有流程',ok:true},{label:'脚本版本已保存',note:versions.value.length ? `${versions.value.length} 个版本` : '待确认',ok:versions.value.length>0},{label:'知识点与 PPT 映射已检查',note:'教师确认后可发布',ok:false},{label:'音频和数字人产物',note:'按需生成',ok:false}])
