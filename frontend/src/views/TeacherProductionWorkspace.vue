...
const steps = computed(() => [
  ...
  { id: 'audio', label: '音频生成', icon: Volume2, state: taskSummary.value.blocking ? 'failed' : taskSummary.value.running ? 'running' : 'pending', stateLabel: taskSummary.value.blocking ? '需恢复' : taskSummary.value.running ? '生成中' : '查看状态', description: '在脚本确认后生成音频。', legacyTitle: '音频生成', legacyDescription: '批量 TTS 任务仍由现有编辑器发起。' },
  { id: 'avatar', label: '数字人生成', icon: Video, state: taskSummary.value.blocking ? 'failed' : taskSummary.value.running ? 'running' : 'pending', stateLabel: taskSummary.value.blocking ? '需恢复' : taskSummary.value.running ? '生成中' : '查看状态', description: '在音频与映射确认后生成数字人内容。', legacyTitle: '数字人生成', legacyDescription: '数字人生成与资产配置仍在原编辑器。' },
])
