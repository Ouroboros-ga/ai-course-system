...
> **2026-09-08 现行路线（NEXT）：**[CS 语料向量化与 RAG 上线实施计划](../docs/phase1/2026-09-08_CS语料向量化与RAG上线实施计划.md)。保留112节点/106关系的精编概念库；大规模语料改走“保存原文 → 切块 → 本地多语言向量 + FTS → 两个 Agent 检索”，不再全量抽取概念/关系。DK1/DK4 修改后复用；DK2取消；DK3退出本路线，详见新计划 §2。
>
> 下文带日期的数量、耗时和内存为历史记录，不代表本轮重新实测。当前代码已有 TeachingAgent 语料参考链，但 Nexus 仍只查精编节点；新索引、统一检索和完整上线属于 CR0–CR6 待实施任务。
...
4. ⏳ 待接线：作为**课程知识图谱的种子候选**导入 `CourseKnowledgeNode` / `GraphRelation`，
   经教师审核门后进入正式图谱快照；
5. ✅ **已落地（2026-09-01，语料层接入检索白名单 + 向量混合检索）**：
   `corpus/build_corpus_index.py` 离线构建 FTS5 索引（contentless，635 万段落 /
   7.05GB，CJK 二元组 + ASCII 词分词），运行期由
   `backend/app/platform/knowledge/discipline_corpus.py` 只读检索；
   `corpus/build_corpus_embeddings.py` 另将**中文高权威子集**（中文维基 CS
   125,474 段 + 教材 4,877 段，共 130,351 段）用本地
   `bge-small-zh-v1.5`（复用 GraphRAG 模型设施）离线嵌入写入 pgvector 表
   ...
   （BM25 + BGE + RRF + Citation 闭包）仍是两条独立链路；
...
## 语料层（corpus/，2026-09-01 登记约 3.50 GB / 3.26 GiB）

精编概念层（上表 JSON）之上新增**语料层**：计算机学科方向的开放许可大规模语料，
存放于 `.corpus_cache/`（本地，不入库），由 `corpus/` 下脚本构建，
统计汇总见 `corpus/manifest.json`。2026-09-01 曾上传并校验旧服务器；现管理地址为
`103.36.223.177`，语料目录为 `/opt/smartcarb/shared/knowledge_corpus/`，旧地址已退租。
本会话前序只读快照：五个原始 JSONL 合计3,501,624,282 bytes，FTS文件7,053,971,456 bytes、
向量表130,351行；本轮没有重新连接服务器。历史约635万段未在该次核对中取得成功计数，
CR0重新盘点后才能用于全量排期。
...
* `build_corpus_embeddings.py`：中文高权威子集（zhwiki-/ostep-/sicp- 前缀，
  130,351 段）本地 BGE 离线嵌入 → pgvector 表 `discipline_corpus_embedding`
  （服务器端构建约 3.1 小时，nice 降优先级不阻断线上服务）。该表是可重建的
  派生数据，不属于 alembic 业务 schema。

诚实边界：

* 语料层检索**已接入 TeachingAgent 学科参考白名单**（见"集成路径"第 5 条），
  FTS 路仅要求 `DISCIPLINE_CORPUS_INDEX_PATH`；向量路另需
  `DISCIPLINE_CORPUS_VECTOR_ENABLED=true` 与已构建的向量表，默认关闭
  fail-closed。当前向量表只覆盖中文高权威子集（教材 + 中文维基 CS），
  英文维基/RFC/arXiv 仅走 FTS；查询侧模型懒加载，每 worker 进程首次
  向量查询约 15s（torch 加载）并常驻约 1.4GB 内存。

* 上表许可为来源登记标签，不是对所有用途的授权结论。CR0保留逐文档许可及出处，
  许可缺失/不符合已批准用途的记录排除并计数；不因“公开可下载”自动认定可任意再分发。
  受版权保护的市售教材全文未纳入。
...
* 本目录提供数据与离线脚本，运行时由 Backend 服务消费。TeachingAgent 已有精编概念及
  语料补充参考调用链；Nexus 当前只查精编概念。代码接线存在不等于服务器开关启用、
  全量覆盖或最终回答正确；本轮未重新验证线上有效配置。
